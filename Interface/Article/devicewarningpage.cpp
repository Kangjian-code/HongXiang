/**************************************************************
 * File Name   : devicewarningform.cpp
 * Author      : ThreeDog
...
#include "ui_devicewarningpage.h"

DeviceWarningPage::DeviceWarningPage(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::DeviceWarningPage)
{
    ui->setupUi(this);
    initInterface();
    initProperty();
...
void DeviceWarningPage::initInterface()
{
    //初始化界面上的几个控件的内容
    //间隔
    ...
    typeMap.insert(3,"同步源未连接");
    typeMap.insert(4,"同步源未连接");
    typeMap.insert(5,"同步源连接但是与用户设置同步方式的不同");

}

void DeviceWarningPage::initProperty()
{
...
void DeviceWarningPage::initTable()
{
    ...
    m_pTableWidget->setColumnWidth(2,360);
    m_pTableWidget->setColumnWidth(3,360);
    m_pTableWidget->setColumnWidth(4,360);
    m_pTableWidget->setColumnWidth(5,360);
...
void DeviceWarningPage::updateTable()
{
    ...
    for(int i = 0 ; i < vect.count() ; i++){
        ...
        QTableWidgetItem *item3 = new QTableWidgetItem(alarm.alarmTime);
        QTableWidgetItem *item4 = new QTableWidgetItem(typeMap.value(alarm.alarmType));
        ...
        this->m_pTableWidget->setCellWidget(i,5,w2);
...
DeviceWarningPage::~DeviceWarningPage()
{
    delete ui;
}

void DeviceWarningPage::checkBoxClicked(bool b)
{
    if(b){
        qDebug()<<"打开所有的复选框";
...
void DeviceWarningPage::on_queryBtn_clicked()
{
    updateTable();
}
