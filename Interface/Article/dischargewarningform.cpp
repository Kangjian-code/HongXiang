/**************************************************************
 * File Name   : dischargewarningform.cpp
 * Author      : ThreeDog
...
#include "ui_dischargewarningform.h"

DischargeWarningForm::DischargeWarningForm(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::DischargeWarningForm)
{
    ui->setupUi(this);
    initInterface();
    initProperty();
...
void DischargeWarningForm::initInterface()
{
    //初始化界面上的几个控件的内容
    //间隔
    ...
    typeMap.insert(0,"正常");
    typeMap.insert(1,"自由金属颗粒");
    typeMap.insert(2,"尖端放电");
    typeMap.insert(3,"悬浮放电  ");
    typeMap.insert(4,"沿面放电");
    typeMap.insert(5,"内部放电");
    levelMap.insert(0,"正常");
    levelMap.insert(1,"轻微放电");
    levelMap.insert(2,"放电明显");

}
void DischargeWarningForm::initProperty()
{
...
void DischargeWarningForm::initTable()
{
    ...
    headText << "" << "间隔" << "监测点"<< "局放类型" << "报警时间" << "报警等级" << "故障类型" << "操作" ;
...
void DischargeWarningForm::updateTable()
{
    ...
    for(int i = 0 ; i < vect.count() ; i++){
        ...
        check->setProperty("id",vect.at(i).id); //将id设置为属性
        h->addWidget(check);
        h->addStretch();
        ...
        QTableWidgetItem *item5 = new QTableWidgetItem(levelMap.value(alarm.alarmLevel));
...
DischargeWarningForm::~DischargeWarningForm()
{
    delete ui;
}

void DischargeWarningForm::checkBoxClicked(bool b)
{
    if(b){
        qDebug()<<"打开所有的复选框";
